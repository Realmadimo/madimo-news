    ...
    <script>
        function goBackOrHome() {
            if (document.referrer) {
                ...
                window.location.href = 'dashboard.php'; // آدرس مقصد
            }
        }
    </script>
                        ...
                        <?php foreach ($genres as $genre): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <!-- Delete Button -->
                                    <a href="?delete=<?= $genre['id'] ?>" class="btn btn-danger btn-sm"
                                    ...
                                    <div class="modal fade" id="editModal<?= $genre['id'] ?>" tabindex="-1"
                                        aria-labelledby="editModalLabel<?= $genre['id'] ?>" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <form method="POST">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="editModalLabel<?= $genre['id'] ?>">Edit
                                                            Genre</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="mb-3">
                                                            <label for="genreName<?= $genre['id'] ?>"
                                                                class="form-label">Genre Name</label>
                                                            <input type="text" name="name" id="genreName<?= $genre['id'] ?>"
                                                                class="form-control"
                                                                value="<?= htmlspecialchars($genre['name']) ?>" required>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary"
                                                            data-bs-dismiss="modal">Cancel</button>
                                                        <button type="submit" name="edit_genre" class="btn btn-primary">Save
                                                            Changes</button>
                                                    </div>
                                                </form>
                                            </div>
